...
def lang(locale: str):
    ...
    kb.add(
        InlineKeyboardButton(
            "[Auto]" if locale is None else "Auto", callback_data=lang_cd.new("0")
        )
    )
    for i in _.available_locales:
        ...
        kb.add(
            InlineKeyboardButton(
                f"[{label}]" if i == locale else label, callback_data=lang_cd.new(i)
            )
        )
    return kb
...
def admin():
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton(_("Settings"), callback_data=settings_cd.new("set")))
    return kb

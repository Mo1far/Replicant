...
async def settings(cq: types.CallbackQuery):
    await cq.answer()
    await cq.message.edit_text(_("Settings"), reply_markup=kb.settings())


@dp.callback_query_handler(settings_cd.filter(set="lang"))
async def show_lang(cq: types.CallbackQuery, locale: str):
    await cq.answer()
    await cq.message.edit_text(_("Choose language"), reply_markup=kb.lang(locale))
...
async def choose_lang(cq: types.CallbackQuery, callback_data: dict, user: User):
    ...
    await cq.message.edit_text(
        _("Settings", locale=lang), reply_markup=kb.settings(locale=lang)
    )
...
async def pagination(cq: types.CallbackQuery, callback_data: dict):
    ...
    await cq.message.edit_text(
        f"Inline Pagination | {page}", reply_markup=kb.page(page, last, words)
    )
